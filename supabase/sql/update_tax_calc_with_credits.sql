-- Update income_tax_calculation view to include tax credits
-- Tax credits (heffingskortingen) reduce the actual tax payable
...
user_deductions AS (
  SELECT
    ...
    -- Sum of all other enabled custom deductions
    COALESCE((
...
tax_credits_calc AS (
  SELECT
    tc.year,
    tc.taxable_income,
    tc.tax_bracket_1 + tc.tax_bracket_2 AS tax_before_credits,
    -- Calculate algemene heffingskorting (with phaseout)
    COALESCE((
      SELECT
        CASE
          WHEN tc.taxable_income <= tcr.phaseout_start THEN tcr.max_amount
          WHEN tc.taxable_income >= tcr.phaseout_end THEN 0
          ELSE GREATEST(0, tcr.max_amount - ((tc.taxable_income - tcr.phaseout_start) * tcr.phaseout_rate / 100))
        END
      FROM tax_credits tcr
      JOIN user_tax_credit_selections utcs ON utcs.credit_id = tcr.id AND utcs.is_enabled = true
      JOIN tax_years ty ON ty.id = tcr.tax_year_id
      WHERE ty.year = tc.year AND tcr.credit_type = 'algemene_heffingskorting'
      LIMIT 1
    ), 0) AS algemene_heffingskorting,
    -- Arbeidskorting placeholder (for now, set to 0, needs complex logic)
    0 AS arbeidskorting
  FROM tax_calc tc
)
SELECT
  ...
  tc.custom_deductions,
  tc.mkb_profit_exemption,
  tc.profit_after_deductions,
  tc.mkb_exemption_amount,
  tcc.taxable_income,
  tc.bracket_1_limit,
  tc.bracket_1_rate,
  tc.tax_bracket_1,
  tc.bracket_2_rate,
  tc.tax_bracket_2,
  tcc.tax_before_credits AS total_income_tax_before_credits,
  tcc.algemene_heffingskorting,
  tcc.arbeidskorting,
  GREATEST(0, tcc.tax_before_credits - tcc.algemene_heffingskorting - tcc.arbeidskorting) AS total_income_tax,
  CASE
    WHEN tc.gross_profit > 0 THEN (GREATEST(0, tcc.tax_before_credits - tcc.algemene_heffingskorting - tcc.arbeidskorting) / tc.gross_profit * 100)
    ELSE 0
  END AS effective_tax_rate,
  tc.gross_profit - GREATEST(0, tcc.tax_before_credits - tcc.algemene_heffingskorting - tcc.arbeidskorting) AS net_profit_after_tax
FROM tax_credits_calc tcc
JOIN tax_calc tc ON tc.year = tcc.year
ORDER BY tcc.year DESC;
